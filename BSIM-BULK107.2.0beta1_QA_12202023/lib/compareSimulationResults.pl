...
#  1.9   12/03/2015      Geoffrey Coram  More debugging info in verbose mode
...
Options:
    ...
    -e                     expect error (no results due error)
    -v                     verbose mode
";
} # End of usage
...
FAIL        (expected simulation *not* to run)
FAIL        (reference results should be empty)
DIFFER      (max rel error is relErr)
MATCH       (simulation stopped as expected)
...
The UNIX utilities spiff and ndiff do toleranced numerical comparisons,
...
($variant=$ARGV[1])=~s/^.*\.//;
$result=&compareResults($ARGV[0],$ARGV[1],$clip,$nDigits,$relTol,$error);
printf "     variant: %-20s(compared to: %-9s) %s\n",$variant,$reference,$result;

sub compareResults {
    use strict;
    my($refFile,$simFile,$clip,$nDigits,$relTol,$expErr)=@_;
    ...
    close(IF);
    if ($expErr) {
        return("FAIL        (reference results should be empty)") if ($#Ref>=1);
        return("FAIL        (expected simulation *not* to run)") if ($#Sim>=1);
        return("MATCH       (simulation stopped as expected)");
    }
